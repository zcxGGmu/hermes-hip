...
- 当前最新阶段提交：`50723af feat: 实现 hermeship 配置模型与 config CLI`。
...
- Milestone 1.3：质量门禁与仓库基础尚未开始。
  ...
  - `cargo clippy --all-targets -- -D warnings` 尚未作为阶段门禁运行并记录。
...
从 `tasks/development-checklist.md` 的 **Milestone 1：Rust 项目骨架与质量门禁** 继续，优先执行 **任务 1.3：质量门禁与仓库基础**。
...
2. 确认当前分支、最新提交和未提交变更：
   ...
   - `git log -3 --oneline`
3. 确认最新阶段提交为 `50723af feat: 实现 hermeship 配置模型与 config CLI`。
4. 从 `tasks/development-checklist.md` 的任务 1.3 继续，先更新 `.gitignore`、rustfmt/clippy 约束说明和 fixture 目录。
5. 运行任务 1.3 验证命令：
   - `cargo fmt --all -- --check`
   - `cargo clippy --all-targets -- -D warnings`
   - `cargo test`
6. 更新 `tasks/development-checklist.md` 的运行状态日志和 `tasks/todo.md` 的 Review。
7. 阶段完成后必须验证并提交，commit 信息使用详细中文，说明变更、验证和影响。
...
- Milestone 1.2：配置模型已经完成并提交，最新阶段提交为 50723af feat: 实现 hermeship 配置模型与 config CLI。
...
- 已通过验证：cargo fmt --all -- --check、cargo test config、cargo run -- config show、cargo test。
...
请从 tasks/development-checklist.md 的 Milestone 1 继续，优先执行任务 1.3：质量门禁与仓库基础：
1. 先更新 tasks/todo.md 写明本阶段计划。
2. 确认当前分支、最新提交和未提交变更：git status --short --branch、git log -3 --oneline。
3. 更新 .gitignore：保留 /target/，增加本地临时文件、日志和测试输出目录，但不要忽略源码、文档、fixture 或 Cargo.lock。
4. 增加 rustfmt/clippy 约束说明，放在 README.md 或 docs/development.md。
5. 新增测试 fixture 目录：tests/fixtures/hermes/、tests/fixtures/privacy/、tests/fixtures/routes/、tests/fixtures/discord/，保留 tests/fixtures/cli/。
6. fixture 目录中不得包含真实 Discord token、真实 Hermes gateway 数据、真实 GitHub/tmux 状态、cookie、secret、完整 prompt、完整对话或 provider request/response body。
7. 运行任务 1.3 验证命令：cargo fmt --all -- --check、cargo clippy --all-targets -- -D warnings、cargo test。
