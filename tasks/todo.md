...
- 已复习：`tasks/lessons.md`。
- 公开材料必须保持 Hermeship 独立叙述，不写 `clawhip`、`template/clawhip`、thin adapter、runtime adapter。
...
- [ ] 确认网站形态：推荐新增独立静态站点目录（例如 `site/`），不引入 npm/Vite/Next/Astro。
- [ ] 设计首页信息架构：Hero、30 秒试跑、ingest/normalize/scrub/route/render/deliver 工作流、能力矩阵、架构图、安装配置、验证边界、Footer。
- [ ] 复用现有资产：`docs/assets/branding/*` 和 `docs/assets/diagrams/*.png`。
- [ ] 使用纯 CSS 设计系统：深色背景、品牌金黄/琥珀、青绿运行态、冷蓝辅助和系统字体/等宽 fallback。
- [ ] 实现响应式与可访问性：移动端导航、可见 focus、减少动画支持、图像 alt、无横向滚动。
- [ ] 验证静态站点：本地打开或启动轻量静态服务器，使用浏览器检查桌面和移动端截图，运行 `git diff --check`。
