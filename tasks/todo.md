...
更新时间：2026-06-16 Milestone 4.1 已完成
...
- 已完成 Router 纯逻辑：新增 `src/router.rs`，支持 event glob、结构化 metadata filter、disabled route、missing target 诊断和 0..N delivery。
- 已将 `hermeship explain` 从 placeholder 替换为本地 route explain：加载当前配置、清洗 payload、构造 typed `EventEnvelope`、打印 route candidates、route 诊断和 delivery target；不调用 daemon、不入队、不渲染、不投递。
- 已补测试：`cargo test router` 覆盖多 route、多 delivery、filter 命中/未命中、disabled route、missing target、无 route、route-level channel/format/template/mention 继承、explain 输出契约和 webhook 诊断脱敏。
- 已处理代码审查反馈：`explain` human/serialized diagnostics 不输出完整 Discord webhook URL，内部 delivery target 仍保留原始 webhook 供后续 dispatcher 使用。
- 已运行验证：`cargo test router`、`cargo run -- explain hermes.agent.started --payload '{"platform":"telegram","session_id":"demo"}'`、`cargo fmt --all -- --check`、`cargo clippy --all-targets -- -D warnings`、`cargo test` 均通过。
- 本阶段未实现 renderer、dispatcher、sink、hook bridge install、install/uninstall lifecycle 或 release preflight；下一阶段继续 Milestone 4.2 Renderer。
